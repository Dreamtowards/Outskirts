...
public final class GuiHawksWindow extends Gui {
    ...
    }).setHeight(32).addOnDraggingListener(e -> {
        if (Gui.isMouseOver(0, 0, Outskirts.getWidth(), Outskirts.getHeight())) {
            GuiHawksWindow.this.setRelativeX(GuiHawksWindow.this.getRelativeX() + (int)e.getDX());
            GuiHawksWindow.this.setRelativeY(GuiHawksWindow.this.getRelativeY() + (int)e.getDY());
        }
    });
    private Gui sizeDrag = addGui(new Gui() {
        ...
        GuiHawksWindow.this.setWidth(GuiHawksWindow.this.getWidth() + (int)e.getDX());
        GuiHawksWindow.this.setHeight(GuiHawksWindow.this.getHeight() + (int)e.getDY());
    });
